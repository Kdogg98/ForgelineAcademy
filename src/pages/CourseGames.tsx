...
import { COURSE_ID, COURSE_TITLE, GAME_SET, isBearingsGamesCourse } from '@/lib/games/bearingsLubricationAlignment';
...
export function CourseGames({ courseId, onNavigate }: CourseGamesProps) {
  ...
  const playable = isBearingsGamesCourse(courseId);
  const [activeGame, setActiveGame] = useState<GameId | null>(playable ? 'g1-crib-call' : null);
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6">
        {!playable && (
          <div className="card border-rok-500/40 bg-navy-950/80 p-5">
            <h2 className="font-display text-lg font-semibold text-white">Wrong course for these drills</h2>
            <p className="mt-2 text-sm text-steel-300">
              Shop-floor games are only playable on {COURSE_TITLE}. Game 1 is not locked behind Game 0 — open the Bearings course to start.
            </p>
            <div className="mt-4">
              <button
                type="button"
                className="btn-primary"
                onClick={() => onNavigate({ name: 'games', courseId: COURSE_ID })}
              >
                Open Bearings games
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {GAME_SET.map((game) => {
            ...
            const unlocked = playable && (game.number <= 1 || previousGamePassed(game.number, GAME_SET, progress));
                ...
                <div className="mt-4">
                  {canLaunch ? (
                    ...
                    >
                      ...
                      {saved?.passed ? `Replay Game ${game.number}` : `Play Game ${game.number}`}
                    </button>
                  ) : (
                    <div className="rounded-md border border-steel-700 bg-navy-950/50 px-3 py-2 text-sm text-steel-400">
                      {game.number <= 1 && !playable
                        ? 'Shop-floor games live on Bearings, Lubrication & Alignment. Open Bearings games to start Game 1.'
                        : !unlocked
