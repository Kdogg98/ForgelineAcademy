...
async function sendEmail(to: string[], subject: string, html: string): Promise<boolean> {
  ...
  const notifyEmail = Deno.env.get('NOTIFY_EMAIL') ?? to[0] ?? 'support@forgelineacademy.com';
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'ForgeLine Academy <noreply@forgelineacademy.com>',
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const notifyEmail = Deno.env.get('NOTIFY_EMAIL') ?? 'support@forgelineacademy.com';
    let subject = '';
    let html = '';
