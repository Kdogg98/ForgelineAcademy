...
import { DEFAULT_PASS_THRESHOLD, saveGameResult } from '@/lib/games/progress';
...
export function BearingTypeBoard({ courseId, userId, onComplete }: BearingTypeBoardProps) {
  ...
  function resetBoard() {
    ...
    setCardTone({});
    setPicked(null);
    setFeedback({ tone: 'info', message: 'Board reset. Match every machine before you submit.' });
    setSubmitted(false);
    setResult(null);
  }
  ...
  function submitAll() {
    ...
    const passed = MACHINES.every((m) => assignments[m.id] === m.correct);
    ...
    setFeedback({
      tone: passed ? 'ok' : 'bad',
      message: passed
        ? `Crib call confirmed. ${score}/${MAX_SCORE}. All five machines are in the right family.`
        : `Score ${score}/${MAX_SCORE}. Need ${DEFAULT_PASS_THRESHOLD}% and every card correct to pass. Reset the red cards and recut the match.`,
    });
    onComplete?.(next);
  }
  ...
  return (
    <GameShell
      ...
      subtitle="Match the crib family to the machine. Drag or tap to assign. All five must be correct to pass."
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        <section className="lg:col-span-4">
          <div className="mb-3 flex items-baseline justify-between">
            ...
            <span className="font-mono text-[10px] text-steel-500">4 families</span>
          </div>
        ...
        <section className="lg:col-span-8">
          <div className="mb-3 flex items-baseline justify-between">
            <h3 className="font-mono text-[11px] uppercase tracking-[0.16em] text-steel-400">Machine cards</h3>
